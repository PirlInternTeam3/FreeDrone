...
def Fully_layer_1(previous_layer):
    ...
    F_W1 = tf.get_variable("F_W1", shape=[FLAGS.F_layer_size_1, FLAGS.F_output_size_1],
...
def Fully_layer_2(previous_layer):
    ...
    F_W2 = tf.get_variable("F_W2", shape=[FLAGS.F_layer_size_2, FLAGS.F_output_size_2],
                           initializer=tf.contrib.layers.xavier_initializer())
...
def Final_layer(previous_layer):
    ...
    Final_W = tf.get_variable("Final", shape=[FLAGS.Final_layer_size, FLAGS.Final_output_size],
                              initializer=tf.contrib.layers.xavier_initializer())
...
if __name__ == "__main__":
    ...
    image_batch, label_batch, file_batch = Data_batch("./train_data/bebop2_train_data.txt")
    bebop2_dataset = pd.read_csv('train_data/bebop2_train_dataset.csv')
    total = len(bebop2_dataset.index)
    ...
    for epoch in range(FLAGS.training_epochs):
        ...
        validate_image_batch, validate_label_batch, validate_file_batch = Data_batch_Validation(
            "./train_data/bebop2_validation_data.txt", 1000)
        coord = tf.train.Coordinator()
        threads = tf.train.start_queue_runners(sess=sess, coord=coord)
        ...
        Accuracy_val = sess.run(accuracy,
                                feed_dict={images: validate_images_, labels: validate_labels_, keep_prob: 1.0})
