...
def convert_yolo_detections(predictions,classes=20,num=2,square=True,side=7,w=1,h=1,threshold=0.02,only_objectness=0):
    boxes = []
...
def do_nms_sort(boxes,total,classes=20,thresh=0.2):
    for k in range(classes):
        for box in boxes:
            box.class_num = k
        sorted_boxes = sorted(boxes, key=functools.cmp_to_key(prob_compare))
        #print(sorted_boxes)
